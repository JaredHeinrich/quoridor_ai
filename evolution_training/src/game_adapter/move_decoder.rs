...
pub fn decode_move(
    nn_output: &Matrix,
    game: &Game,
    play_deterministically: bool
) -> Result<Move> {
    // Validate output matrix dimensions
    if nn_output.rows != 132 || nn_output.columns != 1 {
        return Err(GameAdapterError::InvalidMoveDecoding(nn_output.rows, nn_output.columns).into());
    }
...
mod tests {
    ...
    fn test_deterministic_fallback_to_next_highest() {
        ...
        game.walls.push(Wall::new(Vector::new(4, 7), Orientation::Horizontal));

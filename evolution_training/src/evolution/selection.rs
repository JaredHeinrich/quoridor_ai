...
pub fn select_next_generation(
    ...
    let mut new_generation = Generation {
        agents: Vec::with_capacity(settings.generation_size), generation_index: old_generation.generation_index + 1,
        mutation_rate: old_generation.mutation_rate * (1.0 - settings.mutation_rate_decrease as f64).max(0.0),
    };
...
mod tests {
    ...
    fn test_selection_preserves_top_performers() -> Result<()> {
        ...
        let top_nn_weights: Vec<_> = old_gen.agents.iter()
    ...
    fn test_reactivation_functionality() -> Result<()> {
        ...
        let reactivated_count = new_gen.agents.iter()
            .filter(|agent| agent.neural_network.layer_sizes == vec![10, 10, 5])
            .count();
        assert_eq!(reactivated_count, settings.reactivator_count(), 

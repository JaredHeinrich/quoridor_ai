...
pub fn play_against_benchmark(
    ...
    for move_counter in 0..settings.max_moves_per_player * 2 {
        let current_player = game.current_pawn;
        ...
        if distance_to_goal(&game, 0) == 0 || distance_to_goal(&game, 1) == 0 {
            let winner = if distance_to_goal(&game, 0) == 0 { 0 } else { 1 };
...
mod tests {
    ...
    fn test_forward_agent_handles_blocked_path() -> Result<()> {
        ...
        let wall = quoridor::wall::Wall::new(
            Vector::new(3, 7),
            quoridor::wall::Orientation::Horizontal
        );

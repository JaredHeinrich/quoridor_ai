...
use crate::game_adapter::board_encoder::{encode_board, distance_to_goal};
...
impl TrainingEnvironment {
    ...
    pub fn reset(&mut self) -> Result<()> {
        ...
        println!("Initialized generation 0 with {} agents", self.settings.generation_size);
        Ok(())
    }
    ...
    pub fn run_evolution(&mut self) -> Result<()> {
        ...
        println!("Starting evolutionary training for {} generations", self.settings.number_of_generations);
        ...
        for current_generation_num in 0..self.settings.number_of_generations {
            ...
            println!("─────────────────────────────────────────────────");
            println!("Starting generation {}/{}", self.generation_number, self.settings.number_of_generations);
            ...
            println!("Generation {} completed in {:.2?}", self.generation_number, generation_duration);
        ...
        println!("Training complete after {} generations in {:.2?}", 
                 self.generation_number + 1, total_duration);
    ...
    fn log_generation_results(&mut self) -> Result<()> {
        ...
        let avg_fitness: f64 = generation.agents.iter().map(|a| a.get_fitness()).sum::<f64>() / generation.agents.len() as f64;
    ...
    pub fn play_generation(&mut self) -> Result<()> {
        ...
        println!("Playing {} games in generation {}", num_games, self.generation_number + 1);
    ...
    fn play_game_between_agents(
        ...
        
        {
            ...
            agent0_nn = generation.get_neural_network(agent0_index)
                .ok_or_else(|| anyhow::anyhow!("Agent index {} out of bounds", agent0_index))?
                .clone();
            agent1_nn = generation.get_neural_network(agent1_index)
    ...
    fn play_single_game(&self, neural_network0: &NeuralNetwork, neural_network1: &NeuralNetwork) -> Result<(f64, f64)> {
        // Create a new game
        let mut game = Game::new(self.settings.board_size as i16, self.settings.walls_per_player as i16);
    ...
    fn calculate_rewards(&self, game: &Game, moves_played: usize) -> Result<(f64, f64)> {
        match self.settings.reward_function {
            ...
            crate::game_adapter::reward::RewardFunction::Symmetric => {
                Ok((
    ...
    pub fn evolve_population(&mut self) -> Result<()> {
        ...
        println!("Evolving population for generation {}", self.generation_number + 1);
        
        // Create next generation using selection strategy
        let next_generation = select_next_generation(Some(&mut self.current_generation), &self.settings)?;

...
pub fn plot_fitness_history(environment: &TrainingEnvironment) -> Result<()> {
    ...
    let output_file = format!("{}_fitness_plot.png", environment.settings.log_file.replace(".json", ""));
    ...
    let min_y = environment.fitness_history
        .iter()
        .map(|(_gen, _max, _avg, min)| *min)
        .fold(f64::INFINITY, |a, b| a.min(b))
        .min(0.0);
    
    let max_y = environment.fitness_history
        .iter()
    ...
    chart.draw_series(LineSeries::new(
        environment.fitness_history.iter().map(|(gen, max, _avg, _min)| (*gen as u32, *max)),
        &RED,
    ))?
    .label("Max Fitness")
    ...
    chart.draw_series(LineSeries::new(
        environment.fitness_history.iter().map(|(gen, _max, avg, _min)| (*gen as u32, *avg)),
        &GREEN,
    ))?
    .label("Avg Fitness")
    ...
    // Plot min fitness
    chart.draw_series(LineSeries::new(
        environment.fitness_history.iter().map(|(gen, _max, _avg, min)| (*gen as u32, *min)),
        &BLUE,
    ))?
    .label("Min Fitness")
...
pub fn plot_generation_times(environment: &TrainingEnvironment) -> Result<()> {
    ...
    let output_file = format!("{}_time_plot.png", environment.settings.log_file.replace(".json", ""));
    ...
    let max_y = environment.generation_time_history
        .iter()
        .map(|(_gen, time)| *time)
        .fold(f64::NEG_INFINITY, |a, b| a.max(b))
        * 1.1; // Add 10% margin
    ...
    chart.draw_series(LineSeries::new(
        environment.generation_time_history.iter().map(|(gen, time)| (*gen as u32, *time)),
        &BLUE,
    ))?
    ...
    if environment.generation_time_history.len() >= 3 {
        ...
        for i in 1..environment.generation_time_history.len() - 1 {
            let avg_time = (environment.generation_time_history[i-1].1 + 
                           ...
                           environment.generation_time_history[i+1].1) / 3.0;
...
pub fn plot_diversity_history(environment: &TrainingEnvironment) -> Result<()> {
    ...
    let output_file = format!("{}_diversity_plot.png", environment.settings.log_file.replace(".json", ""));
    ...
    let max_y = environment.diversity_history
        .iter()
    ...
    let mut chart = ChartBuilder::on(&root)
        .caption("Genetic Diversity over Generations", ("sans-serif", 30).into_font())
        .margin(10)
        .x_label_area_size(40)
        .y_label_area_size(60)
        .build_cartesian_2d(0u32..max_gen, min_y..max_y)?;
    ...
    // Plot diversity
    chart.draw_series(LineSeries::new(
        environment.diversity_history.iter().map(|(gen, div)| (*gen as u32, *div)),
        &PURPLE,
    ))?
    ...
    if !environment.fitness_history.is_empty() {
        ...
        let max_fitness = environment.fitness_history
            .iter()
            .map(|(_gen, max, _avg, _min)| *max)
            .fold(f64::NEG_INFINITY, |a, b| a.max(b));
        ...
        if max_fitness > 0.0 {
            ...
            chart.draw_series(LineSeries::new(
                environment.fitness_history.iter()
                    .filter(|(gen, _max, _avg, _min)| *gen < environment.diversity_history.len())
